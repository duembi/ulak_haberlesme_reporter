...
def rapor_tekil_gonder(pdf_yolu: Path, alicilar: list[str], rapor_adi: str, mesaj: str = "") -> bool:
    """Var olan tek bir raporu seçilen alıcılara PDF eki ile gönderir (manuel, tek seferlik)."""
    if not alicilar:
        return False

    konu = f"Ulak Haberleşme Medya Raporu — {rapor_adi}"
    mesaj_html = f"<p style='margin:0 0 16px;color:#2c3e50;font-size:14px;line-height:1.6'>{mesaj}</p>" if mesaj.strip() else ""
...
<body style="font-family:Calibri,Arial,sans-serif;background:#f4f6f8;margin:0;padding:0">
  <div style="max-width:680px;margin:30px auto;background:#fff;border-radius:8px;
    ...
    <div style="background:#1A5276;padding:28px 32px">
      <h1 style="color:#fff;margin:0;font-size:22px">ULAK HABERLEŞME A.Ş.</h1>
      <p style="color:#AED6F1;margin:6px 0 0;font-size:14px">Medya Takip Raporu</p>
    </div>
    <div style="padding:28px 32px">
      {mesaj_html}
      <p style="margin:0;color:#2c3e50;font-size:14px;line-height:1.6">
        "{rapor_adi}" raporu ekte PDF olarak paylaşılmıştır.
      </p>
    </div>
    <div style="background:#EBF5FB;padding:16px 32px;text-align:center">
      <p style="color:#5D6D7E;font-size:12px;margin:0">
        Ulak Haberleşme Medya Takip Sistemi üzerinden gönderilmiştir.
      </p>
    </div>
  </div>
</body>
